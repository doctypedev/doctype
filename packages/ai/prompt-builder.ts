...
- Use proper Markdown formatting (bolding, lists, code blocks)
- Do NOT use markdown headers (e.g. #, ##, ###). Use bolding or lists for structure.
...
CRITICAL - Do NOT generate HTML comments in your output:
- NEVER write comments like <!-- doctype:start -->, <!-- doctype:end -->, or any HTML comments
- HTML comments are reserved for system use only
- Your content will be injected between anchor tags automatically
...
Format the response as clean Markdown ready for injection into documentation files.`;
  ...
  static buildUserPrompt(request: DocumentationRequest, options: GenerateOptions = {}): string {
    ...
    sections.push('7. Do NOT use markdown headers (e.g. #, ##, ###). Use bolding or lists for structure.');
    sections.push('8. Do NOT include HTML comments of any kind.');

    sections.push('');
    sections.push('IMPORTANT: Generate **only** the documentation content in Markdown format.');
    sections.push('- Do NOT include any preamble, meta-commentary, or explanations');
    sections.push('- Do NOT include HTML comments (no <!-- --> tags)');
    sections.push('- Your output will be inserted directly between anchor tags');
  ...
  static buildInitialPrompt(symbolName: string, signature: string, options: GenerateOptions = {}): string {
    ...
    sections.push('6. Do NOT use markdown headers (e.g. #, ##, ###). Use bolding or lists for structure.');
    sections.push('7. Do NOT include HTML comments of any kind.');

    sections.push('');
    sections.push('IMPORTANT: Generate **only** the Markdown documentation content.');
    sections.push('- Do NOT include any preamble or meta-commentary');
    sections.push('- Do NOT include HTML comments (no <!-- --> tags)');
    sections.push('- Your output will be inserted directly between anchor tags');
   ...
   */
  static buildBatchPrompt(items: Array<{ symbolName: string; signatureText: string }>): string {
    const sections: string[] = [];

    sections.push(`# Batch Documentation Generation`);
    sections.push('');
    sections.push(`Generate documentation for the following ${items.length} symbols.`);
    sections.push('');
    ...
    sections.push(`## Instructions`);
    sections.push('');
    sections.push('For EACH symbol, generate documentation that:');
    sections.push('1. Explains the purpose and behavior');
    sections.push('2. Documents all parameters with types');
    sections.push('3. Documents the return type');
    sections.push('4. Includes a usage example');
    sections.push('5. Do NOT use markdown headers (e.g. #, ##, ###) within the documentation content. Use bolding or lists for structure.');
    sections.push('');
    sections.push('Ensure the output matches the requested JSON schema.');
